...
curPrf = '/bwdata/NSDData/Seohee/Orientation/prfsample_Par/';
...
for iregion=1:numregions
    ...
    if length(rois)>1 %combine across ventral and dorsal ROIs
        ...
        for iroi=1:length(rois)
            ...
            nsd.voxFeatResidual{1} = cat(2,nsd.voxFeatResidual{1},oldNsd.voxLenResidual{iroi});
            nsd.voxResidualSplit{1} = cat(2,nsd.voxResidualSplit{1},oldNsd.voxLenResidual{iroi});
            nsd.voxFeatResidualSplit{1} = cat(2,nsd.voxFeatResidualSplit{1},oldNsd.voxLenResidual{iroi});

            nsd.pearsonRfeat{1} = cat(2,nsd.pearsonRfeat{1},oldNsd.pearsonRlen{iroi});
            nsd.pearsonR{1} = cat(2,nsd.pearsonR{1},oldNsd.pearsonR{iroi});
            nsd.r2{1} = cat(2,nsd.r2{1},oldNsd.r2{iroi});
            nsd.r2feat{1} = cat(2,nsd.r2feat{1},oldNsd.r2len{iroi});
            nsd.r2split{1} = cat(2,nsd.r2split{1},oldNsd.r2split{iroi});
            nsd.r2featSplit{1} = cat(2,nsd.r2featSplit{1},oldNsd.r2lenSplit{iroi});
            nsd.aicFeatSplit{1} = cat(2,nsd.aicFeatSplit{1},oldNsd.aiclenSplit{iroi});
            nsd.bicFeatSplit{1} = cat(2,nsd.bicFeatSplit{1},oldNsd.biclenSplit{iroi});
            nsd.voxCoef{1} = cat(2,nsd.voxCoef{1},oldNsd.voxCoef{iroi});
            nsd.voxFeatCoef{1} = cat(2,nsd.voxFeatCoef{1},oldNsd.voxOriCoef{iroi});
            nsd.voxPredFeatCoef{1} = cat(2,nsd.voxPredFeatCoef{1},oldNsd.voxPredOriCoef{iroi});
            nsd.voxFeatPredFeatCoef{1} = cat(2,nsd.voxFeatPredFeatCoef{1},oldNsd.voxOriPredOriCoef{iroi});
            nsd.voxResidFeatCoef{1} = cat(2,nsd.voxResidFeatCoef{1},oldNsd.voxResidOriCoef{iroi});
            nsd.voxFeatResidFeatCoef{1} = cat(2,nsd.voxFeatResidFeatCoef{1},oldNsd.voxOriResidOriCoef{iroi});

            nsd.voxPredFeatR2{1} = cat(2,nsd.voxPredFeatR2{1},oldNsd.voxPredOriR2{iroi});
            nsd.voxFeatPredFeatR2{1} = cat(2,nsd.voxFeatPredFeatR2{1},oldNsd.voxOriPredOriR2{iroi});
            nsd.voxResidFeatR2{1} = cat(2,nsd.voxResidFeatR2{1},oldNsd.voxResidOriR2{iroi});
            nsd.voxFeatResidFeatR2{1} = cat(2,nsd.voxFeatResidFeatR2{1},oldNsd.voxOriResidOriR2{iroi});
    ...
    for  iroi=1:length(rois)%rois=1
        for isplit=1:nsplits
            ...
            keyboard;
            theta = linspace(0,2*pi,numFeatures+1);%for circular calculation
            theta = theta(1:end-1);
            for ivox=1:numvox
    ...
    roiNsdOriR2{iregion} = nsd.r2featSplit{iroi};
    % roiNsdR2{iregion} = nsd.r2split{iroi};
    allaicOriSplit{iregion} = nsd.aicFeatSplit{iroi};
    allbicOriSplit{iregion} = nsd.bicFeatSplit{iroi};
