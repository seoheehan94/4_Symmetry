...
function regressPrfSplit_symmetryAll(isub,visualRegions,type)
...
for visualRegion=visualRegions
    visualRegion
    load(fullfile(boxfolder,['prfSampleStim_',type, '_v' num2str(visualRegion) '_sub' num2str(isub) '.mat']),'prfSampleLevOri',...
    ...
    r2featSplit_reduced_tap = cell(length(rois),1);
    r2featSplit_only_par = cell(length(rois),1);
    r2featSplit_only_mir = cell(length(rois),1);
    r2featSplit_only_tap = cell(length(rois),1);
    ...
    for roinum=1:length(rois)
        ...
        voxFeatResidualSplit_reduced_par{roinum} = NaN(nsplits, nvox(roinum),maxNumTrials);
        voxFeatResidualSplit_reduced_mir{roinum} = NaN(nsplits, nvox(roinum),maxNumTrials);
        voxFeatResidualSplit_reduced_tap{roinum} = NaN(nsplits, nvox(roinum),maxNumTrials);
        ...
        reducedCoef_par{roinum} = zeros(nsplits, nvox(roinum),numLevels*numFeatures);
        reducedCoef_mir{roinum} = zeros(nsplits, nvox(roinum),numLevels*numFeatures);
        reducedCoef_tap{roinum} = zeros(nsplits, nvox(roinum),numLevels*numFeatures);
            ...
            for ivox=1:nvox(roinum)
                ...
                if rank_X < size(voxPrfFeatSample, 2)
                    ...
                    for p = 1:3
                        ...
                        reducedSample(:, p) = []; % Remove the p-th predictor

                        % Run regression without predictor p
                        if p == 1
                            reducedCoef_par{roinum}(isplit,ivox,:) = reducedSample \ voxBetas;
                        elseif p == 2
                            reducedCoef_mir{roinum}(isplit,ivox,:) = reducedSample \ voxBetas;
                        elseif p == 3
                            reducedCoef_tap{roinum}(isplit,ivox,:) = reducedSample \ voxBetas;
                        end
        ...
        for isplit=1:nsplits
            ...
            for ivox=1:nvox(roinum)
                ...
                for p = 1:3
                    ...
                    if p == 1
                        reducedCoef_par{roinum}(isplit,ivox,:) = reducedSample \ voxBetas;
                        voxFeatResidualSplit_reduced_par{roinum}(isplit,ivox,1:numTrials) = voxBetas' - squeeze(reducedCoef_par{roinum}(nsplits-isplit+1,ivox,:))'*reducedSample';
                    elseif p == 2
                        reducedCoef_mir{roinum}(isplit,ivox,:) = reducedSample \ voxBetas;
                        voxFeatResidualSplit_reduced_mir{roinum}(isplit,ivox,1:numTrials) = voxBetas' - squeeze(reducedCoef_mir{roinum}(nsplits-isplit+1,ivox,:))'*reducedSample';
                    elseif p == 3
                        reducedCoef_tap{roinum}(isplit,ivox,:) = reducedSample \ voxBetas;
                        voxFeatResidualSplit_reduced_tap{roinum}(isplit,ivox,1:numTrials) = voxBetas' - squeeze(reducedCoef_tap{roinum}(nsplits-isplit+1,ivox,:))'*reducedSample';
                    end
                end
                ...
                r2featSplit_reduced_par{roinum}(isplit,ivox) = rsquared(voxFeatResidualSplit_reduced_par{roinum}(isplit,ivox,1:sum(splitImgTrials(isplit,:))), roiBetas{roinum}(ivox,imgTrials>0));
                r2featSplit_reduced_mir{roinum}(isplit,ivox) = rsquared(voxFeatResidualSplit_reduced_mir{roinum}(isplit,ivox,1:sum(splitImgTrials(isplit,:))), roiBetas{roinum}(ivox,imgTrials>0));
                r2featSplit_reduced_tap{roinum}(isplit,ivox) = rsquared(voxFeatResidualSplit_reduced_tap{roinum}(isplit,ivox,1:sum(splitImgTrials(isplit,:))), roiBetas{roinum}(ivox,imgTrials>0));
                r2featSplit_only_par{roinum}(isplit,ivox) = r2featSplit{roinum}(isplit,ivox) - r2featSplit_reduced_par{roinum}(isplit,ivox);
                r2featSplit_only_mir{roinum}(isplit,ivox) = r2featSplit{roinum}(isplit,ivox) - r2featSplit_reduced_mir{roinum}(isplit,ivox);
                r2featSplit_only_tap{roinum}(isplit,ivox) = r2featSplit{roinum}(isplit,ivox) - r2featSplit_reduced_tap{roinum}(isplit,ivox);
    ...
    nsd.r2featSplit_only_par = r2featSplit_only_par;
    nsd.r2featSplit_only_mir = r2featSplit_only_mir;
    nsd.r2featSplit_only_tap = r2featSplit_only_tap;
