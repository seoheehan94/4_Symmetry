clear all;
symType = 'tap'; % Choose from: 'par', 'mir', 'tap' 
...
for itype = 1:length(methods)
    type = methods{itype};
    filedir = [filedir_base, 'surfaceData_', symType, '/'];

    for isub = 1:8
        % Load left and right hemisphere volume data
        vol_lh.(type)(:, isub) = load_mgh([filedir, 'sub', num2str(isub), '/', type, 'Brain_sub', num2str(isub), '_lh_fsaverage.mgh']);
        vol_rh.(type)(:, isub) = load_mgh([filedir, 'sub', num2str(isub), '/', type, 'Brain_sub', num2str(isub), '_rh_fsaverage.mgh']);

        % Load R² values for weighting
        R2_lh.(type)(:, isub) = load_mgh([filedir, 'sub', num2str(isub), '/', type, 'BrainR2_sub', num2str(isub), '_lh_fsaverage.mgh']);
        R2_rh.(type)(:, isub) = load_mgh([filedir, 'sub', num2str(isub), '/', type, 'BrainR2_sub', num2str(isub), '_rh_fsaverage.mgh']);
    end
end
...
pairNames = {'contour_medialAxis', 'contour_area', 'medialAxis_area'};
pairs = {'contour', 'medialAxis'; 'contour', 'area'; 'medialAxis', 'area'};
...
for ipair = 1:length(pairs)
    ...
    filedir_save = [filedir_base, 'surfaceData_diff/method/'];
    save_mgh(mean_weighted_diff_lh, [filedir_save, symType, '_lh_', pairNames{ipair}, '_weighted_diff_full.mgh'], M, mr);
    save_mgh(mean_weighted_diff_rh, [filedir_save, symType, '_rh_', pairNames{ipair}, '_weighted_diff_full.mgh'], M, mr);

    save_mgh(R2cutoff_diff_lh, [filedir_save, symType, '_lh_', pairNames{ipair}, '_weighted_diff_R2cut.mgh'], M, mr);
    save_mgh(R2cutoff_diff_rh, [filedir_save, symType, '_rh_', pairNames{ipair}, '_weighted_diff_R2cut.mgh'], M, mr);
end
